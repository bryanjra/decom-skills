import React from 'react';
import {Destacado} from '../templates/Destacado';
import type {AdComponent} from '../types';

// Reference ad for the "destacado" template. A real ad is props-driven the same way:
// every fact comes from `event` / `iglesia`, nothing about the church or the
// date is written here.
export const Ad: AdComponent = ({event, iglesia}) => <Destacado event={event} iglesia={iglesia} />;
